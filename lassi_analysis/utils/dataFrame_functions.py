...
def node2node_distances_dataframe(atomsdf):
    ...
    def node2node_distances(atomsdf:object):
        def distance_generator(atomsdf:object):
            ...
            return
        return numpy.array(list(distance_generator(atomsdf))).reshape(len(atomsdf),len(atomsdf)) #reshape because we return a (len(atomsdf)xlen(atomsdf))x1 array otherwise. So we group len(n) elements at a time
