...
def parseToFile(open_file:str,save_as:str):
    ...
    dimensions = [int(i) for i in re.search("BOX\sBOUNDS.*?\n(\d+\s\d+).*?\n",p,re.DOTALL)[1].split()]
        ...
        for i in h:
            if len(i) == 0:
                ...
                if molecule_id in dicts.keys():
                    dicts[molecule_id][atom_id] = {"x":int(nums[3]),"y":int(nums[4]),"z":int(nums[5])}
                else:
                    dicts[molecule_id] = {}
                    dicts[molecule_id][atom_id] = {"x":int(nums[3]),"y":int(nums[4]),"z":int(nums[5])}
        info_dict[j] = dicts
    ...
    outdict = {"id_to_atom-type":atom_id_map,"structured-data":info_dict,"dimensions":dimensions,"natoms":natoms}
